...
// Modal for Viewing Quiz Leaderboard (when released by Admin)
const StudentLeaderboardModal = ({ quiz, myUserId, onClose }) => {
    const modalRef = useRef(null);
    ...
    return (
        ...
        >
            <div
                ...
                className="bg-[#0e1524] border border-slate-700/90 rounded-3xl max-w-lg w-full shadow-[0_25px_70px_rgba(0,0,0,0.85)] overflow-hidden text-slate-100"
            >
                {/* Header */}
                <div className="bg-gradient-to-r from-amber-600/30 via-yellow-600/30 to-indigo-600/30 px-6 py-5 border-b border-slate-800 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-2xl bg-amber-500/20 border border-amber-500/40 text-amber-400 flex items-center justify-center shrink-0 shadow-lg shadow-amber-900/30">
                            <Trophy className="w-5 h-5 text-amber-400" />
                        </div>
                        <div className="min-w-0">
                            <h3 className="text-base font-black text-white truncate">{quiz.name}</h3>
                            <p className="text-xs text-amber-300/90 font-bold uppercase tracking-wider">Official Quiz Leaderboard</p>
                        </div>
                    </div>
                    <button
                        type="button"
                ...
                <div className="p-5 max-h-[70vh] overflow-y-auto">
                    <LeaderboardPanel quizId={quiz.id} myUserId={myUserId} formatSecs={formatSecs} />
                </div>
            </div>
        </div>
    );
...
const QuizList = ({ user, onLogout }) => {
    ...
    const [leaderboardModalQuiz, setLeaderboardModalQuiz] = useState(null);
        ...
        <div className="min-h-screen bg-[#0b0f17] flex flex-col justify-between font-sans w-full relative overflow-hidden text-slate-100">
            ...
            <div className="w-full flex-1 relative z-10">
                ...
                <header ref={headerRef} className="bg-[#0f172a]/95 backdrop-blur-md border-b border-slate-800/80 sticky top-0 z-40 shadow-[0_4px_20px_rgba(0,0,0,0.3)]">
                    {/* Top Row: Brand & Navigation (Strictly 1 Single Row on Mobile) */}
                    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-2 flex-nowrap relative z-30">
                        {/* Logo & Portal Name */}
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                            ...
                            <div className="flex flex-col justify-center min-w-0">
                                <div className="flex items-center gap-1.5">
                                    <span className="font-black text-sm sm:text-xl tracking-tight text-white leading-tight truncate">
                                        Helix Club Quiz
                                    </span>
                                    <span className="text-[9px] sm:text-[10px] uppercase font-bold tracking-wider px-1.5 sm:px-2 py-0.5 bg-blue-950/80 text-blue-300 border border-blue-800/60 rounded-full shrink-0">
                                        Official
                                    </span>
                                </div>
                                <div className="hidden sm:flex items-center gap-2 text-[11px] font-semibold text-slate-400 leading-tight">
                                    <span>Assessment & Contest Portal</span>
                                    <span>•</span>
                        ...
                        <nav className="hidden md:flex items-center gap-1.5 bg-slate-900/90 p-1 rounded-2xl border border-slate-800 text-xs font-bold shrink-0">
                            <button
                        ...
                        {/* Right: User Profile & Logout (Single Line Controls) */}
                        <div className="flex items-center gap-2 shrink-0">
                            {/* Avatar Button */}
                            <button
                                onClick={() => setShowProfilePanel(p => !p)}
                                className="flex items-center gap-1.5 sm:gap-2.5 bg-slate-900/90 px-2 sm:px-3 py-1 sm:py-1.5 rounded-2xl border border-slate-800 shadow-2xs hover:border-slate-700 hover:bg-slate-800/80 transition-all cursor-pointer shrink-0"
                                title="View Profile"
                            >
                                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#4169e2] to-indigo-500 text-white font-bold text-xs flex items-center justify-center shadow-xs shrink-0">
                                                ...
                                                {isSubmitted ? (
                                                    <div className="flex flex-col gap-2 items-end">
                                                        ...
                                                        <div className="flex flex-wrap items-center justify-end gap-1.5">
                                                            {/* 1. Solution & Answer Key Button (If released by Admin) */}
                                                            {(quiz.isAnswerKeyPublic === true || quiz.answerKeyPublic === true) ? (
                                                                ...
                                                                <span className="px-2.5 py-1 bg-slate-900/90 text-slate-400 rounded-xl text-[10px] font-semibold border border-slate-800 flex items-center gap-1">
                                                                    <Lock className="w-3 h-3 text-slate-500" /> Key: Admin Pending
                                                                </span>
                                                            )}

                                                            {/* 2. Leaderboard View Button (If released by Admin) */}
                                                            {(quiz.isLeaderboardPublic === true || quiz.leaderboardPublic === true) ? (
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setLeaderboardModalQuiz(quiz)}
                                                                    className="px-3.5 py-1.5 bg-amber-950/90 hover:bg-amber-800 border border-amber-700/80 text-amber-200 hover:text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-amber-900/30 cursor-pointer flex items-center gap-1.5"
                                                                >
                                                                ...
                                                                <span className="px-2.5 py-1 bg-slate-900/90 text-slate-400 rounded-xl text-[10px] font-semibold border border-slate-800 flex items-center gap-1">
                                                                    <Lock className="w-3 h-3 text-slate-500" /> Leaderboard: Admin Pending
                                                                </span>
                                                            )}
                                                        </div>
                                                    </div>
            ...
            {leaderboardModalQuiz && (
                <StudentLeaderboardModal
                    quiz={leaderboardModalQuiz}
                    myUserId={user?.uid}
                    onClose={() => setLeaderboardModalQuiz(null)}
                />
            )}
